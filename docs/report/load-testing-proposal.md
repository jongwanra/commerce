...
작년 선착순 쿠폰 발급 이벤트 시점에, 약 1초 안에 1,500명의 요청이 유입되며 시스템 장애가 발생한 이력이 있습니다.
...
- 부하 패턴:
    1. Ready: VUser(가상 유저) 3,000명, 수량 1,000개를 가지고 있는 쿠폰 데이터을 사전에 세팅합니다.
    2. Fire: 14:00:00 정각에 3,000명이 동시에 발급 버튼을 클릭합니다.
    3. Check
        - 준비된 1,000장이 정확히 소진되었는가?
        - 나머지 2,000명은 "마감되었습니다" 응답을 정상적으로 받았는가?
...
- App Container: `CPU: 2.0`, `Memory: 4GB` (Resource Limit 설정)
- DB Container: MySQL 8.0
- Redis Container: 분삭락 처리를 위한 캐시 캐시 서버
